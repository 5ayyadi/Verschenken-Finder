...
def scrap_category_location(soup: BeautifulSoup) -> dict[str, str]:
    ...
    if breadcrumb_tag:
        ...
        if len(links) >= 1:
            state_city_tag = breadcrumb_tag.find("h1").find(
                "span", class_="breadcrump-summary")
            ...
            category_link = breadcrumb_tag.find(
                "a", class_="breadcrump-link", title=False)
            if category_link:
                category = category_link.text
                category_tag = breadcrumb_tag.find("h1").find(
                    "span", class_="breadcrump-leaf")
...
def find_offers(category_id: str = None, city_id: str = None, max_price: int = 0) -> list[dict]:
    ...
    Args:
        ...
        max_price (int, optional): maximum price to filter by. 0 means free items only. Defaults to 0.
    ...
    while True:
        ...
        for offer in offers:
            ...
            if price:
                ...
                # Handle free items (Zu verschenken)
                ...
                # Handle priced items if max_price > 0
                elif max_price > 0 and "€" in price_text:
                    # Extract price number from text like "20 €" or "€20"
                    import re
                    price_match = re.search(r'(\d+)', price_text)
                    if price_match:
                        item_price = int(price_match.group(1))
                        if item_price <= max_price:
                            offer_item = offer.find_parent(
                                "article", class_="aditem")
                            parsed_offer = parse_verschenken_offer(offer_item)
                            if parsed_offer.id in existing_offer_ids:
                                continue
                            # stop it if today - cutoff_date older than offer_date
                            offer_date = date.fromisoformat(
                                parsed_offer.offer_date)
                            if offer_date < date.today() - timedelta(days=CUTOFF_DATE):
                                return results
                            parsed_offer.location = location
                            parsed_offer.category = category
                            ...
                            # If price is higher than max_price, stop searching (sorted by price)
                            return results

                # For VB (negotiable) prices, stop if max_price is 0 (free items only)
                elif "VB" in price_text and max_price == 0:
                    return results

                # If no conditions met and we're looking for free items only, stop
                elif max_price == 0:
                    return results
            else:
                return results
